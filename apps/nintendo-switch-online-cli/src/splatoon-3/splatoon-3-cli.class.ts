...
Bullet token: \u001b[90mexpires ${String(new Date(bulletToken.expires))}
    ...
    for (const battle of battles.data.latestBattleHistories.historyGroups.nodes[0].historyDetails.nodes) {
      ...
      if (!filenames.includes(`${idForFilename}.json`)) {
        ...
        await nsoCli.stream.wrapSpinner(
          nsoCli.config.saveJsonToFile('splatoon-3/battles', idForFilename, battleData.data.vsHistoryDetail),
          `Saving battle`
        );
      }
    }
  ...
  async saveLatestSalmonRunShifts() {
    ...
    const filenames = await nsoCli.config.getFilesInDirectory('splatoon-3/salmon-run-shifts');
    let newShifts = 0;
    for (const group of shifts.data.coopResult.historyGroups.nodes) {
      for (const shift of group.historyDetails.nodes) {
        ...
        if (!filenames.includes(`${idForFilename}.json`)) {
          ...
          await nsoCli.stream.wrapSpinner(
            nsoCli.config.saveJsonToFile('splatoon-3/salmon-run-shifts', idForFilename, shiftData.data.coopHistoryDetail),
            `Saving shift`
          );
        }
      }
    ...
    console.log('');
    console.log(newShifts === 0 ? 'No new shifts' : `Done, \u001b[1;32m${newShifts}\u001b[0m new shifts were saved`);
    console.log(`\u001b[1;32m${filenames.length + newShifts}\u001b[0m total shifts saved`);
  }
}
