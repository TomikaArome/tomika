...
describe('Permission', () => {
  ...
  it('should be able to get the permission object from the full label', () => {
    const editInfoPermission = helper.getPermission(
      'perm.admin.manage-users.edit-info'
    );
  ...
  it('should be able to get the parent of a permission', () => {
    ...
    const manageUsersPermission = helper.getPermission(
      'perm.admin.manage-users'
    );
    expect(helper.getParentPermission(editInfoPermission)).toBe(
  ...
  it('should be able to test if a permission is an ancestor of another permission', () => {
    ...
    expect(
      PermissionHelper.isAncestorOf(adminLabel, editInfoLabel)
    ).toBeTruthy();
    expect(
      PermissionHelper.isAncestorOf(editInfoLabel, adminLabel)
    ).toBeFalsy();
    expect(
      PermissionHelper.isAncestorOf(editInfoLabel, editInfoLabel)
    ).toBeFalsy();
    expect(
      PermissionHelper.isAncestorOf(commentLabel, editInfoLabel)
    ).toBeFalsy();
  });
  ...
  it('should be able to simplify an array of permission-tree', () => {
    ...
    expect(PermissionHelper.simplifyFullLabelsArray(originalArray)).toEqual([
      'perm.admin',
      'perm.post',
    ]);
  });
  ...
  it('should be able to check if a permission has been granted from an array of permission-tree', () => {
    ...
    expect(
      PermissionHelper.hasPermission(grantedPermissions, editInfoLabel)
    ).toBeTruthy();
    expect(
      PermissionHelper.hasPermission(grantedPermissions, commentLabel)
    ).toBeTruthy();
    expect(
      PermissionHelper.hasPermission(grantedPermissions, rateLabel)
    ).toBeFalsy();
  });
});
