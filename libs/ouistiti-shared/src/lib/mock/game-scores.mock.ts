...
const defaultPlayerIds = ['41fb-BS1I3', '8mk8HmaF3y', 'yfcw75pt8r', 'C_r83VNGVt'];

export const getGameScoresMock = (playerIds: string[] = []): RoundScores[] => {
  if (!playerIds[0]) { playerIds[0] = defaultPlayerIds[0]; }
  if (!playerIds[1]) { playerIds[1] = defaultPlayerIds[1]; }
  if (!playerIds[2]) { playerIds[2] = defaultPlayerIds[2]; }
  if (!playerIds[3]) { playerIds[3] = defaultPlayerIds[3]; }
  ...
  const incompleteGame: RoundScores[] = completeGame.map((roundScore) => {
    return roundScore.roundNumber <= 14 ? roundScore : {
      ...roundScore,
