...
export class ScoreboardComponent implements AfterViewInit {
  ...
  getPlayerScoresInOrder(playerScores: PlayerScore[]): Partial<PlayerScore>[] {
    return this.playersInOrder.map((player) =>
      playerScores.find((score) => score.playerId === player.id)
    );
  }
  ...
  isScoreHigherThanLast(playerId: string, roundNumber: number): boolean {
    return (
      this.getCumulativeScore(playerId, roundNumber) >
      (roundNumber === 1
        ? 0
        : this.getCumulativeScore(playerId, roundNumber - 1))
    );
  }
  ...
  getCellHoverClass(
    ...
    return {
      'line-hover':
        (this.roundHover !== -1 && this.roundHover === roundNumber) ||
        (this.playerHover !== '' && this.playerHover === playerId),
      'cell-hover':
        this.roundHover === roundNumber && this.playerHover === playerId,
    };
  }

  getCellClass(
    roundScore: RoundScores,
    playerScore: Partial<PlayerScore>
  ): { [key: string]: boolean } {
    return {
      'score-is-higher':
        roundScore.playerScores &&
        playerScore.pointDifference &&
        this.isScoreHigherThanLast(
  ...
  getPlayerFromId(playerId: string): PlayerInfo {
    return this.playersInOrder.find(
      (player: PlayerInfo) => playerId === player.id
    );
  }

  private scrollToRoundElement(roundNumber: number) {
    let elRef: ElementRef = this.roundHeaderElements.get(
      roundNumber
    ) as ElementRef;
    if (!elRef) {
      elRef = this.roundHeaderElements.last as ElementRef;
    }
    (elRef.nativeElement as HTMLDivElement).scrollIntoView({
      behavior: this._scrollToRoundSetBeforeViewInit > -1 ? 'auto' : 'smooth',
    });
  }
