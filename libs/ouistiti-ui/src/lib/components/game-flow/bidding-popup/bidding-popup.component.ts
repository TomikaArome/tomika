...
export class BiddingPopupComponent {
  ...
  get maxBid(): number {
    return this.roundInfo.cards.filter(
      (card: OwnedAndKnownCardInfo) => card.ownerId === this.selfId
    ).length;
  }
  ...
  get self(): PlayerInfo {
    return (
      this.players.find((player: PlayerInfo) => player.id === this.selfId) ??
      null
    );
  }
  ...
  get trumpCard(): TrumpCardInfo {
    return this.roundInfo.cards.find(
      (c: TrumpCardInfo) => c.isTrumpCard
    ) as TrumpCardInfo;
  }

  get startingPlayer(): PlayerInfo {
    return this.players.find(
      (p: PlayerInfo) => p.id === this.roundInfo?.currentPlayerId
    );
  }
  get startingPlayerColour(): PlayerColour {
    return this.startingPlayer?.colour;
  }
  get startingPlayerSymbol(): PlayerSymbol {
    return this.startingPlayer?.symbol;
  }

  get startingPlayerNickname(): string {
    return this.startingPlayer?.id === this.selfId
      ? 'You'
      : this.startingPlayer?.nickname;
  }
