...
export class LobbyScreenComponent implements OnDestroy {
  ...
  takenNicknames$ = this.playerList$.pipe(map((players: PlayerInfo[]) =>
  ...
  onPlayerSettingsChanged(player: PlayerInfo, params: PlayerUpdateParams) {
    ...
    if (params.colour !== player.colour) { paramsToEmit.colour = params.colour; shouldEmit = true; }
    if (params.symbol !== player.symbol) { paramsToEmit.symbol = params.symbol; shouldEmit = true; }
    if (shouldEmit) {
  ...
  omitColourFromTakenColours(colour: PlayerColour, taken: PlayerColour[]): PlayerColour[] {
