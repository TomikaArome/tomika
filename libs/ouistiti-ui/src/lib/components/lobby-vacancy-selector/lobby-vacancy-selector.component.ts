...
export class LobbyVacancySelectorComponent {
  ...
  get orderedPlayers(): PlayerInfo[] {
    return this.lobby.players.sort(
      (playerA: PlayerInfo, playerB: PlayerInfo) => {
        if (playerA.vacant && !playerB.vacant) {
          return -1;
        } else if (!playerA.vacant && playerB.vacant) {
          return 1;
        }
  ...
  clickPlayer(player: PlayerInfo) {
    if (player.vacant) {
      ...
      if (this.lobby.passwordProtected) {
        params.password = (
          this.passwordField.nativeElement as HTMLInputElement
        ).value;
      }
