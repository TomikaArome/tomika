...
export class BreakPoint {
  ...
  private get allAcknowledged(): boolean {
    ...
    return (
      values.length > 0 &&
      values.reduce((acc: boolean, curr: boolean) => acc && curr, true)
    );
  }
  ...
  get info(): BreakPointInfo {
    ...
    if (Object.keys(this.acknowledgements).length > 0) {
      info.acknowledgements = { ...this.acknowledgements };
    }
    if (this.buffer) {
      info.buffer = this.buffer.info;
    }
    return info;
  }
  ...
  setTimer(durationInMs: number, cancelOnTimeout = false) {
    if (!this.resolved && !this.cancelled) {
      if (durationInMs > 0) {
        ...
        timer(durationInMs)
          .pipe(takeUntil(merge(this.ended$, this.timerReset$)))
          .subscribe(() => {
            if (cancelOnTimeout) {
