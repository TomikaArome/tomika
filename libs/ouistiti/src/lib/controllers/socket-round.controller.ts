...
export class SocketRoundController {
  private stopIncludingRoundCompleted$ = merge(this.stop$, this.round.completed$);

  private get stop(): MonoTypeOperatorFunction<unknown> { return takeUntil(this.stopIncludingRoundCompleted$); }
  ...
  emitInitialRoundStatus() {
    this.controller.emit('roundInfo', this.round.infoKnownToPlayer(this.controller.player.id));
  }

  subscribeStatusChanged() {
    this.round.statusChanged$.pipe(takeUntil(this.stop$)).subscribe((status: RoundStatus) => {
      ...
      switch (status) {
        ...
        case RoundStatus.END_OF_TURN:
          payload = {
            status,
            breakPoint: this.round.breakPoint?.info ?? null,
            affectedCards: this.round.cards.filter((card: Card) => card.playedOnTurn === this.round.currentTurnNumber).map((card: Card) => card.info)
          }
          break;
