...
export class NsoApp {
  ...
  static init(args?: NsoAppArgs): NsoApp {
    if (!NsoApp.instance) {
      NsoApp.instance = new NsoApp(args.userAgent, args.version ?? null, args.versionCheckInterval ?? VERSION_CHECK_INTERVAL_DEFAULT);
    }
  ...
  static get(): NsoApp {
    if (!NsoApp.instance) {
      throw new NsoError('Initiate the singleton instance of NsoApp with NsoApp.init() first', NsoErrorCode.NSO_APP_INSTANCE_NOT_INITIATED);
    }
    return NsoApp.instance;
  }

  private constructor(readonly userAgent: string, private _version: string, private versionCheckInterval) {
  ...
  async getVersion(): Promise<string> {
    if (this._version && +new Date() < this.lastCheckedVersion + this.versionCheckInterval) { return this._version; }
    const operation = new NsoOperation(NsoOperationType.GET_NSO_APP_VERSION, 'Fetching version of the NSO app');
